...
This is a subtopic of the `Data and Database` workflow.  It covers all handling of the detection data, from the files downloaded in the field to appending the database.
...
## 3. Create a new .vdb in that subfolder, following the naming conventions below.
 ...
 - Once all downloads are complete, create a subfolder within the parent folder titled with the range of download dates covered and a "_dc" suffix, if it has not already been done.  Example:
...
This is done using the `ybt::ybt_db_append()` function.


##   7. Back up updated database to cloud and/or external storage

Currently the database is stored on Dropbox.